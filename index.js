...
function search_artist(event) {
    event.preventDefault();
    const form_d = new FormData(document.querySelector('#main_section[data-index = "7"] form'));
    fetch("search_artist.php?q="+encodeURIComponent(form_d.get('search_artist'))).then(onResponse).then(onArtistJson);
}